# -*- coding: utf-8 -*-
"""lstm-based VAE-Copy3.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/104XcJioPKjs9mPoUqJ8ZX02I9SbnbSdT
"""
...
batch_size = 100
loss_weights = np.zeros((batch_size,MAX_SEQUENCE_LENGTH,NB_WORDS))
for i in range(batch_size):
    for j in range(MAX_SEQUENCE_LENGTH):
        loss_weights[i,j] = weights
...
def sent_generator(TRAIN_DATA,n):
    for i in range(0, len(TRAIN_DATA), n):
        yield [TRAIN_DATA[i:i + n],TRAIN_DATA[i:i + n]]

nb_epoch=1000
n_steps = 1100/batch_size #we use the first 800000
for counter in range(nb_epoch):
    print('-------epoch: ',counter,'--------')
    vae.fit_generator(sent_generator(data_train,batch_size),
                          steps_per_epoch=n_steps, epochs=1)
...
for counter in range(nb_epoch):
    print('-------epoch: ',counter,'--------')
    vae.fit_generator(sent_generator(multi_hot,batch_size),
                          steps_per_epoch=n_steps, epochs=1)
...
patient_id = [i for i in patient_event.keys()]

len(sent_encoded)

encoding = {}
for i in range(len(sent_encoded)):
  encoding[patient_id[i]] = sent_encoded[i]

data_val = data_1[12600:]

sent_encoded = encoder.predict(data_val,batch_size= 1)

key = 12600
for i in range(len(sent_encoded)):
  encoding[patient_id[key]] = sent_encoded[i]
  key+=1
...
encoding_df.to_csv('patient_encoding.df')